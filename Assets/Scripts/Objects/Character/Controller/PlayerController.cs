            ...
            if (hit.collider != null)
            {
                switch(hit.collider.tag)
                {
                    ...
                    case "Character":
                        ...
                        if (charC.IsDead())
                        {
                            var contB = new ContainerBackpack(charC.gameObject);
                            var container1Args = new ContainerArgs{
                                itemsContainer = contB
                            };
                            OnContainerOpen?.Invoke(container1Args);
                        }
                        break;
                    case "Container":
                        var containerC = hit.collider.gameObject.GetComponent<ContainerObject>();
                        var container2Args = new ContainerArgs{
                            itemsContainer = containerC.containerBackpack
                        };
