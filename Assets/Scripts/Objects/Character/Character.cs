...
public class Character : HittableObject
{
    ...
    {
        if (CharacterBackpack.TryAddItem(item))
        {
    ...
    IEnumerator AttackPrimaryCoroutine(Vector3 dir)
    {
        ...
        if (objectHit != null)
        {
            if (objectHit.TryGetComponent(out HittableObject obj))
            {
                obj.GetHit(dir, finalhit.point);
            } 
            if (objectHit.TryGetComponent(out Character character))
            {
